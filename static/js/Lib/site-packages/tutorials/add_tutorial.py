...
class AddTutorial:
    ...
    def save_changes_click(self,e,base_validator:BaseValidation=None):
        ...
        conf=confirm('Add tutorial?')
        if conf:
            self.loader.show_loader()
            jq.post('/modules/api_cable/add_tutorial_cable.php',data_val,self.add_tutorial_con_ok).fail(self.add_tutorial_con_failed)

    def add_tutorial_con_failed(self,res,status,xhr):
        self.loader.close_loader()
        alert('Connection to server not established')
        #loader.close_loader()

    def add_tutorial_con_ok(self,res,status,xhr):
        self.loader.close_loader()
        if status=='success':
            result=json.loads(res)
            if result['status'] != 'success':
