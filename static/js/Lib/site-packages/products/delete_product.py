...
class DeleteProduct:
    def __init__(self) -> None:
        self.cable='/modules/api_cable/delete_product_cable.php'
        self.__onload=None
        self.loader=Loader()
        ...
        if status == 'success':
            try:
                ...
                if self.__onload is not None:
                    self.__onload(DeleteProductResponse(**js))
            except:
                if self.__onload is not None:
                    self.__onload(DeleteProductResponse(
                        status='error',
    ...
    def process_delete(self,service_id=0):
        if service_id == 0:
            if self.__onload is not None:
                self.__onload(DeleteProductResponse(
                    status='failed',
                    message='Please select product to delete'
                ))
                return
        data={
            'delete_service':'ok',
